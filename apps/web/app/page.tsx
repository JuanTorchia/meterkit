...
const copy = {
  en: {
    ...
    provider: "Provider",
    publish: "Publish an endpoint",
    price: "Set 0.01 USDC per request",
    agent: "AI agent",
    request: "Call the protected API",
    policy: "Validate price and recipient",
    chain: "Solana",
    settle: "Settle directly",
    receipt: "Finalize a public receipt",
    sdk: "Developer first",
  ...
  es: {
    ...
    title: (
      <>
        Convierte una llamada API en
        <br />
        <em>un pago directo en USDC.</em>
      </>
    ),
    intro:
      "Agrega cobro por solicitud a una API o herramienta MCP en minutos. El cliente paga directamente a tu wallet; MeterKit nunca recibe los fondos.",
    ...
    outcomeBody:
      "El agente recibe el precio, valida su política, paga USDC de prueba exacto y reintenta. Tu servicio entrega el resultado sólo después de liquidar.",
    provider: "Proveedor",
    publish: "Publica un endpoint",
    price: "Fija 0,01 USDC por solicitud",
    agent: "Agente de IA",
    request: "Llama a la API protegida",
    policy: "Valida precio y destinatario",
    chain: "Solana",
    settle: "Liquida directamente",
    receipt: "Finaliza un recibo público",
    sdk: "Para desarrolladores",
    sdkTitle: "Protege la ruta. Conserva tu stack.",
    sdkBody:
      "MeterKit es middleware, no una capa de custodia. Conserva tu API, wallet y relación con el cliente.",
  ...
  "pt-BR": {
    ...
    title: (
      <>
        Transforme uma chamada de API em
        <br />
        <em>um pagamento direto em USDC.</em>
      </>
    ),
    intro:
      "Adicione cobrança por requisição a uma API ou ferramenta MCP em minutos. O cliente paga diretamente à sua carteira; MeterKit nunca recebe os fundos.",
    ...
    outcomeBody:
      "O agente recebe o preço, valida sua política, paga o USDC de teste exato e tenta novamente. Seu serviço entrega o resultado somente após a liquidação.",
    provider: "Provedor",
    publish: "Publique um endpoint",
    price: "Defina 0,01 USDC por requisição",
    agent: "Agente de IA",
    request: "Chame a API protegida",
    policy: "Valide preço e destinatário",
    chain: "Solana",
    settle: "Liquide diretamente",
    receipt: "Finalize um recibo público",
    sdk: "Para desenvolvedores",
    sdkTitle: "Proteja a rota. Mantenha seu stack.",
    sdkBody:
      "MeterKit é middleware, não uma camada de custódia. Mantenha sua API, carteira e relação com o cliente.",
    ...
    <main id="main-content">
      ...
      <section className="productStory" id="product">
        ...
        <div className="roleGrid">
          <article>
            <span>01 · {text.provider}</span>
            <h3>{text.publish}</h3>
            <p>{text.price}</p>
          </article>
          <article>
            <span>02 · {text.agent}</span>
            <h3>{text.request}</h3>
            <p>{text.policy}</p>
          </article>
          <article>
            <span>03 · {text.chain}</span>
            <h3>{text.settle}</h3>
            <p>{text.receipt}</p>
          </article>
        </div>
      ...
      <section className="codeSection compactCode" id="docs">
        <div>
          <span className="kicker">{text.sdk}</span>
          <h2>{text.sdkTitle}</h2>
          <p>{text.sdkBody}</p>
