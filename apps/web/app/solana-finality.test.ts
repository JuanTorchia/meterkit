...
function rpcResponse(status: string | null, err: unknown = null) {
  return new Response(JSON.stringify({
    jsonrpc: "2.0",
    result: { value: status ? [{ confirmationStatus: status, err }] : [null] },
...
describe("revocation finality", () => {
  it("waits until the signature is finalized", async () => {
    ...
    await expect(waitForFinalizedSignature("signature", "https://rpc.test", fetcher, {
      attempts: 2,
      intervalMs: 0,
  ...
  it("fails when the transaction has an onchain error", async () => {
    await expect(waitForFinalizedSignature(
      "signature",
      "https://rpc.test",
      async () => rpcResponse("finalized", { InstructionError: [0, "Custom"] }),
      { attempts: 1 },
