...
try {
  ...
  await page
    .waitForFunction(
      (minimum) =>
        globalThis.document.querySelectorAll(".transactions .row").length >=
        minimum,
      minimumReceipts,
      { timeout: 30_000 },
    )
  ...
  const evidence = await page.evaluate(() => ({
    ...
    receiptRows:
      globalThis.document.querySelectorAll(".transactions .row").length,
    finalizedRows: Array.from(
      globalThis.document.querySelectorAll(".transactions .row"),
    ).filter((row) => row.textContent?.toLowerCase().includes("finalized"))
      .length,
    explorerLinks: globalThis.document.querySelectorAll(
      ".transactions a[href*='explorer.solana.com']",
    ).length,
    horizontalOverflow:
  ...
  const passed =
    evidence.receiptRows >= minimumReceipts &&
