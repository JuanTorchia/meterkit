...
run("production dependency audit", "pnpm", [
  "audit",
  "--prod",
  "--audit-level=high",
]);
...
run("build web container", "docker", [
  ...
  "Dockerfile.web",
  "--build-arg",
  "NEXT_PUBLIC_GATEWAY_URL=https://meterkit-api.juanchi.dev",
  "--tag",
  `meterkit-web:${imageTag}`,
  ".",
]);
for (const image of [
  `meterkit-gateway:${imageTag}`,
  ...
  run(
    ...
    [
      ...
      "-v",
      "meterkit-trivy-cache:/root/.cache/trivy",
...
if (process.env.SOLANA_PRIVATE_KEY && process.env.MERCHANT_WALLET) {
  run(
    "live x402 devnet settlement and replay",
    "pnpm",
  ...
  checks.push({
    ...
    status: "skipped",
    summary:
      "Requires applicant-controlled SOLANA_PRIVATE_KEY and MERCHANT_WALLET; no secret was loaded.",
  });
}
...
if (process.env.SOLANA_KEYPAIR_PATH && process.env.MERCHANT_WALLET) {
  ...
  checks.push({
    ...
    status: "skipped",
    summary:
      "Requires applicant-controlled SOLANA_KEYPAIR_PATH and MERCHANT_WALLET; no keypair was loaded.",
  });
}
...
const report = {
  ...
  factualScope:
    "Automated internal verification; it is not an external pilot or revenue evidence.",
  ...
  redaction:
    "Secrets, bearer tokens, private keys and full transaction signatures are excluded.",
};
...
function sanitize(value) {
  return value
    ...
    .replace(
      /\[(?:\s*\d{1,3}\s*,){63}\s*\d{1,3}\s*\]/g,
      "[REDACTED_PRIVATE_KEY]",
    )
    .replace(
      /\b[1-9A-HJ-NP-Za-km-z]{64,88}\b/g,
      (match) => `${match.slice(0, 6)}…${match.slice(-6)}`,
    );
}
