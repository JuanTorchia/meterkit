...
if (challengeResponse.status !== 402) throw new Error(`Expected HTTP 402, received ${challengeResponse.status}`);
...
if (!requirement?.network || !requirement.asset || !requirement.amount || !requirement.payTo) {
...
if (!paymentsResponse.ok) throw new Error(`Public receipts returned ${paymentsResponse.status}`);
const payments = await paymentsResponse.json();
const receipts = payments.filter((payment) => payment?.status === "finalized");
if (receipts.length < 1) throw new Error("No finalized public receipt available");
...
if (conversion.status !== 0) throw new Error(`ffmpeg failed: ${conversion.stderr.slice(-1200)}`);
await rm(rawDir, { recursive: true, force: true });
...
async function play(page, scene, data, seconds) {
  await page.evaluate(({ nextScene, nextData }) => {
    ...
    function renderScene(name, d) {
      ...
      if (name === "settlement") return `${head}${copy(d)}
      ...
      if (name === "receipt") return `${head}${copy(d)}
      ...
      if (name === "workspace") return `${head}${copy(d)}
