...
if (challengeResponse.status !== 402) {
  throw new Error(
    `Public endpoint did not return HTTP 402 (${challengeResponse.status})`,
  );
}
...
const requirement = Array.isArray(challenge.accepts)
  ? challenge.accepts[0]
  : undefined;
...
if (!paymentsResponse.ok)
  throw new Error(`Public payments failed (${paymentsResponse.status})`);
const payments = await paymentsResponse.json();
if (!Array.isArray(payments))
  throw new Error("Public payments response is not an array");
const finalized = payments.filter((payment) => payment?.status === "finalized");
if (finalized.length < 3)
  throw new Error("At least three finalized receipts are required");
const latest = finalized[0];
if (
  typeof latest?.explorerUrl !== "string" ||
...
try {
  ...
  await scene(
    ...
    6,
    "MeterKit",
    "Charge for APIs and MCP tools in USDC — without custody.",
  );
  ...
  await scene(
    ...
    "Existing Node.js endpoints can return x402 payment requirements.",
  );
  ...
  await scene(
    ...
    8,
    "Live HTTP 402",
    "The browser just fetched network, mint, amount and recipient from the protected endpoint.",
  );
  ...
  await scene(
    page,
    10,
    "Previously finalized evidence",
    "This synthetic devnet receipt matches the product, network and amount. No new payment is claimed.",
  );
  ...
  await scene(
    ...
    `${finalized.length} finalized payments indexed with Explorer links.`,
  );

  await page.goto(latest.explorerUrl, {
  ...
  await scene(
    ...
    `Transaction ${short(latest.signature, 28)} — independently verifiable.`,
  );

  await page.goto(`${webUrl}/agent/allowances`, {
    waitUntil: "networkidle",
    timeout: 30_000,
  });
  ...
  await page
    .locator("body")
    .evaluate(() => globalThis.scrollTo({ top: 0, behavior: "instant" }));
  await scene(
    page,
    5,
    "Solana Project Scout",
    "A useful MCP tool: public-source project reports, free preview, then 0.02 test USDC.",
  );
  await scene(
    page,
    7,
    "MeterKit",
    "Open source · x402 · subscriptions · MCP · no custody · no token",
  );
  await clearOverlay(page);
  await wait(1);
...
if (conversion.status !== 0)
  throw new Error(`ffmpeg failed: ${conversion.stderr.slice(-1000)}`);
await rm(rawDir, { recursive: true, force: true });
...
async function setOverlay(page, title, detail, mode) {
  await page.evaluate(
    ({ title: nextTitle, detail: nextDetail, mode: nextMode }) => {
      ...
      if (strong)
        Object.assign(strong.style, {
          display: "block",
          color: "#c8f56c",
          fontSize: nextMode === "terminal" ? "16px" : "24px",
          marginBottom: "8px",
        });
      if (body)
        Object.assign(body.style, {
          whiteSpace: "pre-wrap",
          fontSize: nextMode === "terminal" ? "16px" : "18px",
          lineHeight: "1.5",
          fontFamily: nextMode === "terminal" ? "monospace" : "inherit",
        });
...
async function wait(seconds) {
  await new Promise((resolve) =>
    globalThis.setTimeout(resolve, seconds * speed * 1000),
  );
}
