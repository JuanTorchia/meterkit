...
The repository includes the SDK and middleware, deterministic initializer,
searchable English/Spanish docs, Express/Next/Hono/MCP examples, PostgreSQL
receipt index, Next.js dashboard, paying example client, Solana Project Scout,
...
The maintained framework examples are Express (`examples/express-quickstart`),
Next.js App Router (`examples/next-route-quickstart`), Hono
(`examples/hono-quickstart`) and paid MCP (`examples/mcp-scout`). Their exact
supported versions are generated by `pnpm compatibility:verify`.
...
El harness `pnpm benchmark` registra commit, ambiente, concurrencia,
percentiles locales/externos, outcomes y ejecuciones duplicadas. Sus resultados
internos no constituyen un SLA. `pnpm world-class:evidence` genera el paquete
profesional y deja release/health como `not-run` cuando faltan tag, aprobación o
URL explícita.
...
El grant fue aprobado por 200 USDG. Esto es financiación aprobada, no ingreso de
clientes ni prueba de adopción. Al 2026-08-10 no se afirman pilotos externos,
clientes ni revenue comercial.
