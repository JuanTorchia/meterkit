...
export async function verifyEndpoint(
  ...
  try {
    ...
    checks.push({
      ...
      ...(response.status === 402
        ? {}
        : { error: `expected 402, received ${response.status}` }),
    });
    ...
    checks.push({
      ...
      ...(requirement.mint === SOLANA_DEVNET_USDC_MINT
        ? {}
        : { error: "unsupported USDC mint" }),
    });
    checks.push(
      policyCheck(
        "network matches policy",
        policy.network,
        requirement.network,
      ),
    );
    checks.push(
      policyCheck("USDC mint matches policy", policy.mint, requirement.mint),
    );
    checks.push(
      policyCheck(
...
export function parsePolicy(value: unknown): PilotPolicy {
  ...
  if (unknown.length)
    throw new Error(
      `policy contains unknown field${unknown.length === 1 ? "" : "s"}: ${unknown.join(", ")}`,
    );
  ...
  if (input.maxAmountAtomic !== undefined)
    output.maxAmountAtomic = requiredAtomic(input.maxAmountAtomic);
  if (input.allowLocalhost !== undefined) {
    if (typeof input.allowLocalhost !== "boolean")
...
async function parseEndpoint(
  ...
  if ((localName || loopback) && !allowLocalhost) {
    throw new Error(
      "localhost endpoints are blocked; set policy.allowLocalhost to true for local development",
    );
  }
  if (localName || loopback) return url.toString();
  if (url.protocol !== "https:") {
    throw new Error(
      "public endpoints must use https; http is allowed only for explicit localhost development",
    );
  }
  if (isBlockedAddress(hostname))
    throw new Error(
      "endpoint resolves to a private, loopback, link-local or reserved address",
    );
...
function requiredAtomic(value: unknown) {
  ...
  if (!/^\d+$/.test(amount) || BigInt(amount) <= 0n)
    throw new Error("amount must be a positive atomic integer");
  return amount;
}
...
function requiredAddress(value: unknown, name: string) {
  ...
  if (!/^[1-9A-HJ-NP-Za-km-z]{32,44}$/.test(address))
    throw new Error(`${name} is not a Solana address`);
  return address;
}
...
): NonNullable<PilotReport["requirement"]> {
  return {
  ...
  return (
    candidates.find(
      (candidate) =>
        ...
        (!policy.maxAmountAtomic ||
          BigInt(candidate.amountAtomic) <= BigInt(policy.maxAmountAtomic)),
    ) ??
    candidates.find((candidate) => candidate.scheme === "exact") ??
    candidates[0]!
  );
}
...
function report(
  endpoint: string,
  checks: PilotCheck[],
  requirement?: PilotReport["requirement"],
): PilotReport {
  return {
    schemaVersion: 1,
    kind: "meterkit-external-endpoint-readiness",
    ...
    notice:
      "No wallet, private key, seed phrase, payment proof or mainnet asset was used. This verifies readiness, not a completed settlement.",
  };
}
