...
describe("pilot endpoint verifier", () => {
  ...
  it("does not pass when required policy values are omitted", async () => {
    ...
    expect(
      report.checks
        .filter((check) => check.evidence?.enforced === false)
    ...
    const report = await verifyEndpoint(
      ...
      async () =>
        new Response(null, {
          status: 402,
          headers: {
            "PAYMENT-REQUIRED": Buffer.from(JSON.stringify(invalid)).toString(
              "base64",
            ),
          },
        }),
  ...
  it("rejects a policy and challenge outside supported Solana devnet USDC", async () => {
      ...
      accepts: [
        {
          ...acceptedRequirement,
          network: "not-solana",
          asset: acceptedRequirement.payTo,
        },
      ],
    };
    const report = await verifyEndpoint(
    ...
    expect(
      report.checks.find(
        (check) => check.name === "network is supported Solana devnet",
      )?.ok,
    ).toBe(false);
    expect(
      report.checks.find(
        (check) => check.name === "asset is supported devnet USDC",
      )?.ok,
    ).toBe(false);
  });
  ...
  it("ignores malformed alternatives when a later method satisfies policy", async () => {
    const alternatives = {
      ...requirement,
      accepts: [{ scheme: "exact", amount: "not-atomic" }, acceptedRequirement],
    };
    const report = await verifyEndpoint(
      "https://api.example.test/premium",
      {
  ...
  it("parses only known, correctly typed policy fields", () => {
    ...
    expect(() => parsePolicy({ allowLocalhost: "yes" })).toThrow(
      "allowLocalhost must be a boolean",
    );
