...
import { MemoryPaymentStore, SOLANA_DEVNET, productSchema } from "@meterkit/core";
import {
  ...
  it("challenges, settles once, records receipt and rejects replay", async () => {
    ...
    const officialFacilitator: FacilitatorClient = {
      async getSupported() {
        return {
          kinds: [{
            x402Version: 2, scheme: "exact", network: SOLANA_DEVNET,
      ...
      async verify() {
        if (used) return { isValid: false, invalidReason: "payment_already_settled" };
        return { isValid: true, payer: product.payTo };
      },
      async settle(_payload, requirements) {
        used = true;
        return {
          success: true, payer: product.payTo, transaction: "official-signature",
          network: SOLANA_DEVNET, amount: requirements.amount,
        };
      },
    };
    ...
    app.get("/premium", createX402Middleware({
      product, store, facilitatorClient: officialFacilitator, rpcUrl: false,
    ...
    try {
      ...
      const required = JSON.parse(Buffer.from(requiredHeader!, "base64").toString("utf8"));
      expect(required.accepts[0]).toMatchObject({
        network: SOLANA_DEVNET, amount: "10000",
        asset: product.assetMint, payTo: product.payTo,
      });
      ...
      const paid = await fetch(url, { headers: { "PAYMENT-SIGNATURE": signature } });
      expect(paid.status).toBe(200);
      expect(paid.headers.get("PAYMENT-RESPONSE")).toBeTruthy();
      expect(await store.has("official-signature")).toBe(true);
      const replay = await fetch(url, { headers: { "PAYMENT-SIGNATURE": signature } });
...
describe("independent Solana settlement validation", () => {
  ...
  const transaction = (overrides?: {
    mint?: string; recipient?: string; credited?: string; err?: unknown;
  }) => ({
    result: {
      meta: {
        err: overrides?.err ?? null,
        preTokenBalances: [
          { accountIndex: 1, mint: product.assetMint, owner: payer, uiTokenAmount: { amount: "20000" } },
          { accountIndex: 2, mint: overrides?.mint ?? product.assetMint,
            owner: overrides?.recipient ?? product.payTo, uiTokenAmount: { amount: "5000" } },
        ],
        postTokenBalances: [
          { accountIndex: 1, mint: product.assetMint, owner: payer, uiTokenAmount: { amount: "10000" } },
          { accountIndex: 2, mint: overrides?.mint ?? product.assetMint,
  ...
  const validate = async (body: unknown) => new SolanaSettlementValidator(
    "https://rpc.example",
    vi.fn(async () => new Response(JSON.stringify(body), { status: 200 })),
    1,
  ).validate({
    signature: "signature", payer, payTo: product.payTo,
    mint: product.assetMint, amountAtomic: product.priceAtomic,
  });
  ...
  it.each([
    ...
    ["wrong mint", transaction({ mint: "WrongMint111111111111111111111111111111111" })],
    ["wrong recipient", transaction({ recipient: "WrongRecipient1111111111111111111111111111" })],
    ["failed transaction", transaction({ err: { InstructionError: [0, "Custom"] } })],
...
function harness(settlement: Settlement) {
  const store = new MemoryPaymentStore();
  const facilitator: Facilitator = { settle: vi.fn(async (_header, requirements) => {
  ...
  const request = { header: vi.fn(() => Buffer.from('{"payment":"signed"}').toString("base64")) } as any;
  const response = {
    code: 200, body: undefined as any, headers: {} as Record<string, unknown>,
    status(code: number) { this.code = code; return this; },
    json(body: unknown) { this.body = body; return this; },
    set(name: string, value: unknown) { this.headers[name] = value; return this; },
  } as any;
...
describe("MeterKit middleware security", () => {
  ...
  it("settles once and rejects a duplicate receipt", async () => {
    const h = harness({ success: true, transaction: "sig-1", payer: product.payTo, network: SOLANA_DEVNET });
    const next = vi.fn();
  ...
  it.each(["wrong amount", "wrong mint", "wrong recipient"])("rejects %s reported by facilitator", async () => {
    const h = harness({ success: false });
    await h.middleware(h.request, h.response, vi.fn());
    expect(h.response.code).toBe(402);
  });
  ...
  it("rejects wrong network", async () => {
    const h = harness({ success: true, transaction: "sig-2", payer: product.payTo, network: "solana:mainnet" });
    await h.middleware(h.request, h.response, vi.fn());
