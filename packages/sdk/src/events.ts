...
export const paymentLifecycleEventSchema = z.object({
  schemaVersion: z.literal(1),
  type: z.enum(["policy_evaluated", "payment_required", "settled", "rejected", "unknown"]),
...
export type PaymentEventSink = (event: PaymentLifecycleEvent) => void | Promise<void>;

export async function emitSafely(sink: PaymentEventSink | undefined, event: PaymentLifecycleEvent) {
  if (!sink) return;
  try {
