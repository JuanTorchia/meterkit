...
describe("runPaymentPolicies", () => {
  it("preserves ordered decisions and denies enforced deny", async () => {
    const first = { id: "first", evaluate: vi.fn(async () => ({
      policyId: "first", provider: "fixture", outcome: "allow" as const,
      reasonCodes: ["OK"], evaluatedAt: new Date().toISOString(),
    })) };
    const second = { id: "second", evaluate: vi.fn(async () => ({
      policyId: "second", provider: "fixture", outcome: "deny" as const,
      reasonCodes: ["RISK_HIGH"], evaluatedAt: new Date().toISOString(),
    })) };
    ...
    expect(result.decisions.map((item) => item.policyId)).toEqual(["first", "second"]);
  });

  it("applies explicit error policy on timeout", async () => {
    const evaluator = {
      id: "slow",
      evaluate: vi.fn(async (_input: unknown, signal: AbortSignal) =>
    ...
    expect(result.decisions[0]).toMatchObject({ outcome: "error", onErrorApplied: "allow" });
  });
});
