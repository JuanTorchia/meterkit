...
export function createWebacyPolicy(
  options: WebacyPolicyOptions,
  ...
  if (
    ...
    base.username ||
    base.password ||
    !OFFICIAL_ORIGINS.has(base.origin)
  ) {
  ...
  if (!(warnAt >= 0 && warnAt < denyAt && denyAt <= 100))
    throw new Error("WEBACY_THRESHOLDS_INVALID");
  const maxBytes = options.maxResponseBytes ?? 32_768;
  if (maxBytes < 1_024 || maxBytes > 65_536)
    throw new Error("WEBACY_SIZE_LIMIT_INVALID");
  ...
  return {
    ...
    async evaluate(
      ...
      const url = new URL(
        `/addresses/${encodeURIComponent(input.payer)}`,
        base,
      );
      url.searchParams.set("chain", "sol");
      url.searchParams.set(
        "modules",
        "security_essentials,solana_specific,sanctions_compliance",
      );
      ...
      if (Number.isFinite(declared) && declared > maxBytes)
        throw new Error("WEBACY_RESPONSE_TOO_LARGE");
      ...
      const outcome =
        data.overallRisk >= denyAt
          ? "deny"
          : data.overallRisk >= warnAt
            ? "warn"
            : "allow";
      const reason =
        outcome === "deny"
