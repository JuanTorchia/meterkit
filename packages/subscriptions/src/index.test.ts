...
describe("allowance policy", () => {
  it("requires a positive cap and future expiry", () => {
    ...
    expect(
      assertAllowancePolicy({ maxAtomic: 1_000_000n, expiresAt: future })
        .maxAtomic,
    ).toBe(1_000_000n);
    expect(() =>
      assertAllowancePolicy({ maxAtomic: 0n, expiresAt: future }),
    ).toThrow("ALLOWANCE_AMOUNT_INVALID");
    expect(() =>
      assertAllowancePolicy({
        maxAtomic: 1n,
        expiresAt: new Date("2020-01-01"),
      }),
    ).toThrow("ALLOWANCE_EXPIRED");
  });
  ...
  it("rejects overly broad agent allowances by default", () => {
    ...
    ).toThrow("ALLOWANCE_AMOUNT_EXCEEDS_POLICY");
    expect(() =>
      assertAllowancePolicy(
        {
    ...
    ).toThrow("ALLOWANCE_DURATION_EXCEEDS_POLICY");
    expect(() =>
      assertAllowancePolicy(
        {
          maxAtomic: 1_000_000n,
    ...
    ).toThrow("ALLOWANCE_PERIOD_INVALID");
  });
  ...
  it("builds canonical fixed allowance and monthly plan instructions", async () => {
    ...
    const plan = await buildMonthlyPlan({
      ...
      planId: 1n,
      metadataUri: "https://meterkit.dev/plans/pro.json",
    });
  ...
  it("builds recurring, subscribe, and every explicit revocation instruction", async () => {
    ...
    expect(
      [delegation, subscription, all].every(
        (instruction) =>
          instruction.programAddress === subscriptionsIntegration.programId,
      ),
    ).toBe(true);
  });
  ...
  it("builds the native 30-day pull and cancellation instructions", async () => {
    ...
    const subscription = address(
      "9NXuBzJ3EQV4CuxpSVELD3t1bs5xZ6ocfGvwjFDbCZUG",
    );
    const mint = address("4zMMC9srt5Ri5X14GAgXhaHii3GnPAEERYPJgZJDncDU");
    ...
    expect(
      [transfer, cancel, abandoned].every(
        (instruction) =>
          instruction.programAddress === subscriptionsIntegration.programId,
      ),
    ).toBe(true);
  ...
  it("serializes fixed, recurring, plan and subscribe transactions for wallets", async () => {
    ...
    expect(
      transactions.every((transaction) => transaction.byteLength > 100),
    ).toBe(true);
