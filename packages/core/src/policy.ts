...
const forbiddenMetadataKeys = /^(authorization|cookie|password|privatekey|secret|seed|signature|token)$/i;
...
export type PaymentPolicyConfiguration = z.infer<typeof paymentPolicyConfigurationSchema>;
...
export const policyDecisionSchema = z.object({
  ...
  metadata: z.record(z.string().max(64), metadataValueSchema)
    .refine((value) => Object.keys(value).every((key) => !forbiddenMetadataKeys.test(key)), {
