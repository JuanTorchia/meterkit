...
const initSignature = await send(await buildInitSubscriptionAuthority({
  owner: signer,
  mint,
  tokenProgram,
  userAta,
...
const createSignature = await send(await buildFixedAllowance({
  owner: signer,
  mint,
  delegate,
  maxAtomic,
...
const revokeSignature = await send(buildRevokeDelegation({
  owner: signer,
  delegationAccount,
}));
const delegationAfterRevoke = await rpc.getAccountInfo(delegationAccount, {
...
const closeAuthoritySignature = await send(await buildRevokeAllForMint({
  owner: signer,
  mint,
  tokenProgram,
}));
