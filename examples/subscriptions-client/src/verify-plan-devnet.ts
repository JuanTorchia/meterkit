...
const initAuthority = await send(
...
if (
  createdPlan.data.data.terms.amount !== amountAtomic ||
...
const subscribe = await send(
  await buildSubscribe({
    subscriber: signer,
...
if (
  subscribed.data.header.delegator !== signer.address ||
...
const pull = await send(
  await buildTransferSubscription({
    caller: signer,
    subscriber: signer.address,
    plan,
    subscription,
...
if (cancelled.data.expiresAtTs === 0n)
  throw new Error("SUBSCRIPTION_NOT_CANCELLED");
...
const accountAfterRevoke = await rpc
  .getAccountInfo(subscription, {
    commitment: "confirmed",
    encoding: "base64",
  })
  .send();
if (accountAfterRevoke.value !== null)
  throw new Error("SUBSCRIPTION_STILL_EXISTS");
  ...
  `${JSON.stringify(
    {
      ...
      transactions: Object.fromEntries(
        Object.entries(transactions).map(([key, signature]) => [
          key,
          explorer(signature),
        ]),
      ),
...
async function readKeypair() {
  const parsed: unknown = JSON.parse(
    await readFile(required("SOLANA_KEYPAIR_PATH"), "utf8"),
  );
  if (
