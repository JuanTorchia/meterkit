...
const repository = {
  name: "kit",
  full_name: "solana-foundation/kit",
  description: "SDK",
  html_url: "https://github.com/solana-foundation/kit",
  homepage: null,
  license: { spdx_id: "MIT" },
  default_branch: "main",
...
describe("Solana Project Scout", () => {
  it("uses only the expected public GitHub endpoints and cites fresh data", async () => {
    ...
    expect(report).toContain(
      "https://api.github.com/repos/solana-foundation/kit",
    );
  ...
  it("rejects arbitrary URLs and malformed repository names", async () => {
    const request = vi.fn();
    await expect(
      fetchProject("https://evil.example/repo", request),
    ).rejects.toThrow("Formato de repositorio inválido");
  ...
  it("fails closed on oversized and rate-limited GitHub responses", async () => {
    ...
    await expect(
      fetchProject("solana-foundation/kit", oversized as typeof fetch),
    ...
    await expect(
      fetchProject("solana-foundation/kit", limited as typeof fetch),
