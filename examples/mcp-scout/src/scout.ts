...
export async function fetchProject(
  ...
  if (!repoResponse.ok)
    throw new Error(
      `GitHub respondió ${repoResponse.status} para ${repository}`,
    );
  const repo = repoSchema.parse(await readBoundedJson(repoResponse));
  const release = releaseResponse.ok
    ? releaseSchema.parse(await readBoundedJson(releaseResponse))
    : null;
  return {
    name: repo.name,
...
async function safeGithubFetch(
  ...
  try {
    ...
    if (!allowNotFound && response.status === 404)
      throw new Error("Repositorio público no encontrado");
    return response;
  } finally {
    clearTimeout(timer);
  }
