...
| Overspend concurrente de allowance           | reserva SQL atómica; consume tras settlement durable y release sólo ante fallo inequívoco                                             | `agent-budget.test.ts`, integración DB |
| Payload de pago en observabilidad            | recibos enlazan fingerprints SHA-256 acotados; payload y firmas completas quedan fuera de logs                                        | tests SDK/evidence                     |
| Correlation ID malicioso                     | allowlist de caracteres/longitud; valores inválidos se reemplazan por UUID                                                            | `http-policy.test.ts`                  |
| Borrado de metadata por otro tenant          | export/delete resuelven sesión y filtran por owner; estado onchain no se borra                                                        | `server.test.ts`                       |
| OAuth GitHub replay o account takeover       | estado aleatorio hasheado, consumo único, expiración y vínculo a wallet autenticada; GitHub nunca autoriza pagos                      | `github-auth.test.ts`, DB              |
...
- El propietario puede exportar o borrar metadata alojada de allowances. El
  borrado no modifica ni oculta el estado onchain y no autoriza a otro tenant.
- Reservas de allowance vencidas liberan capacidad mediante limpieza
  transaccional; no se eliminan reservas todavía válidas.
